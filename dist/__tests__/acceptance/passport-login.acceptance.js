...
describe('example-passport-login acceptance test', () => {
    ...
    before(async function setupApplication() {
        this.timeout(6000);
        server = await (0, __1.startApplication)(oauth2Providers);
        client = (0, testlab_1.supertest)(server.webApp);
    });
    ...
    describe('User login scenarios', () => {
        ...
        context('Scenario 1: Signing up as a NEW user', () => {
            ...
            it('signup as new user with loopback app', async () => {
                ...
                (0, testlab_1.expect)(redirectUrl).to.equal('/login');
            });
            it('login to loopback app', async () => {
                ...
                (0, testlab_1.expect)(Cookie).to.containEql('session');
            });
            ...
            it('check if user was registered', async () => {
                ...
                (0, testlab_1.expect)(users.length).to.eql(1);
                (0, testlab_1.expect)(users[0].email).to.eql('test@example.com');
            });
        ...
        context('Scenario 2: Link an external profile with a local user', () => {
            ...
            it('call is redirected to third party authorization url', async () => {
                ...
                (0, testlab_1.expect)(url.parse(oauthProviderUrl).pathname).to.equal('/oauth/dialog');
            });
            it('call to authorization url is redirected to oauth providers login page', async () => {
                var _a;
                const response = await (0, testlab_1.supertest)('').get(oauthProviderUrl).expect(302);
                providerLoginUrl = response.get('Location');
                loginPageParams = (_a = url.parse(providerLoginUrl).query) !== null && _a !== void 0 ? _a : '';
                (0, testlab_1.expect)(url.parse(response.get('Location')).pathname).to.equal('/login');
            });
            /**
            ...
            it('login page redirects to authorization app callback endpoint', async () => {
                ...
                const response = await (0, testlab_1.supertest)('http://localhost:9000')
                    .post('/login_submit')
                    .send(qs_1.default.stringify(params))
                    .expect(302);
                callbackToLbApp = response.get('Location');
                (0, testlab_1.expect)(url.parse(callbackToLbApp).pathname).to.equal('/api/auth/thirdparty/oauth2/callback');
            });
            it('callback url contains access code', async () => {
                (0, testlab_1.expect)(url.parse(callbackToLbApp).query).to.containEql('code');
            });
            it('access code can be exchanged for token', async () => {
                ...
                const response = await client
                    ...
                    .expect(302);
                (0, testlab_1.expect)(response.get('Location')).to.equal('/auth/account');
                const setCookie = response.get('Set-Cookie');
                if (setCookie === null || setCookie === void 0 ? void 0 : setCookie.length) {
                    Cookie = setCookie[0].split(';')[0];
                }
            ...
            it('check if profile is linked to existing user', async () => {
                ...
                (0, testlab_1.expect)(profiles === null || profiles === void 0 ? void 0 : profiles.length).to.eql(1);
                (0, testlab_1.expect)(profiles[0].profile).to.eql({
                    emails: [{ value: 'test@example.com' }],
                });
                (0, testlab_1.expect)(profiles[0].provider).to.eql('custom-oauth2');
            });
        });
        /**
         ******************************************************************
        ...
        context('Scenario 3: Sign Up (create a new user) via an external profile', () => {
            ...
            it('call to authorization url is redirected to oauth providers login page', async () => {
                var _a;
                const response = await (0, testlab_1.supertest)('')
            ...
            it('login page redirects to authorization app callback endpoint', async () => {
                ...
                const response = await (0, testlab_1.supertest)('http://localhost:9000')
                    ...
                    .expect(302);
                callbackToLbApp = response.get('Location');
                (0, testlab_1.expect)(url.parse(callbackToLbApp).pathname).to.equal('/api/auth/thirdparty/facebook/callback');
            });
            it('callback url contains access code', async () => {
                (0, testlab_1.expect)(url.parse(callbackToLbApp).query).to.containEql('code');
            });
            ...
            it('check if a new user was registered', async () => {
                ...
                const users = response.body;
                (0, testlab_1.expect)(users.length).to.eql(1);
                (0, testlab_1.expect)(users[0].email).to.eql('usr1@lb.com');
            });
        });
    });
});
